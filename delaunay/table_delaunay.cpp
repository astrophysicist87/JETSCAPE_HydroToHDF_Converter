...
//    John Burkardt
//
int generate_delaunay_2d( const vector<vector<double> > & vertices, vector<vector<double> > & faces )
{ 
  int base = 0;
  int node_dim;
  ...
  node_dim = (vertices.begin())->size();
  node_num = vertices.size();
  node_xy = new double [ node_dim * node_num ];
//cout << "Check: " << node_dim << "   " << node_num << endl;
  int idx = 0;
  ...
  //i4mat_write ( triangle_filename, triangle_order, triangle_num, triangle_node );
  faces.clear();
  faces = vector<vector<double> > ( triangle_num, vector<double>( triangle_order ) );

  idx = 0;
  for (int i = 0; i < triangle_num; i++)
  for (int j = 0; j < triangle_order; j++)
	faces[i][j] = triangle_node[idx++];
